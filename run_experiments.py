...
def experiment1_detection(dataset, results_dir='results'):
    ...
    # Optimize parameters (use subset for speed)
    print("\nOptimizing Viola-Jones parameters...")
    ...
    best_params, _ = optimize_parameters(
        [train_images[i] for i in subset_indices],
        [train_bboxes[i] for i in subset_indices],
        scale_factors=[1.05, 1.1, 1.15, 1.2],
        min_neighbors_list=[3, 4, 5, 6]
    )
...
def experiment2_whole_images(dataset, results_dir='results'):
    ...
    print("\n" + "="*70)
    print("EXPERIMENT II: RECOGNITION ON WHOLE IMAGES")
    print("="*70)
    
    # Load and split data
    train_images, train_ids, _, train_indices = dataset.load_split_data('train', crop_faces=False)
    
    # Create gallery/query split
    identity_images = defaultdict(list)
    for img, identity, idx in zip(train_images, train_ids, train_indices):
        identity_images[identity].append((img, idx))
    
    gallery_images, gallery_ids = [], []
    query_images, query_ids = [], []
    
    for identity, images in identity_images.items():
        if len(images) >= 2:
            gallery_images.append(images[0][0])
            gallery_ids.append(identity)
            for img, _ in images[1:]:
                query_images.append(img)
                query_ids.append(identity)
    
    print(f"Gallery: {len(gallery_images)} images, Query: {len(query_images)} images")
    ...
    for extractor in extractors:
        ...
        gallery_features = [extractor.extract(img) for img in tqdm(gallery_images, desc="Gallery")]
        query_features = [extractor.extract(img) for img in tqdm(query_images, desc="Query")]
...
def experiment3_full_pipeline(dataset, exp1_results, results_dir='results'):
    ...
    print("\n" + "="*70)
    print("EXPERIMENT III: FULL PIPELINE (DETECTION + RECOGNITION)")
    print("="*70)
    ...
    # Create gallery/query split from detected faces
    identity_images = defaultdict(list)
    for img, identity, idx in zip(detected_faces, detected_ids, detected_indices):
        identity_images[identity].append((img, idx))
